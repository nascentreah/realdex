...
@section('content')
    <form id="campaignForm"
        action="{{ isset($campaign) ? route('admin.mailwiz.campaigns.update', $campaign->id) : route('admin.mailwiz.campaigns.store') }}"
        method="POST">
        @csrf
        @if (isset($campaign))
            @method('PUT')
                    ...
                    <div class="card-body space-y-5">
                        ...
                        <div>
                            ...
                            <select id="template_id" name="template_id"
                                ...
                                <option value="" disabled readonly selected>{{ __('Select Template') }}
                                </option>
                        ...
                        <div>
                            ...
                            <input id="campaignName" name="name" type="text"
                                value="{{ $campaign->name }}"class="form-control @if (isset($disabled)) opacity-50 @endif"
                        ...
                        <div>
                            ...
                            <input id="campaignSubject" name="subject" type="text"
                                value="{{ $campaign->subject }}"class="form-control @if (isset($disabled)) opacity-50 @endif"
                                required @if (isset($disabled)) disabled @endif>
                        </div>
                        @error('subject')
                            <span class="text-danger">{{ $message }}</span>
                        ...
                        <div>
                            ...
                            <input id="campaignSpeed" name="speed" type="text"
                                value="{{ $campaign->speed }}"class="form-control @if (isset($disabled)) opacity-50 @endif"
                                required @if (isset($disabled)) disabled @endif>
                        </div>
                        @error('speed')
                            <span class="text-danger">{{ $message }}</span>
...
@section('page-scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Pre-select the users
            @foreach ($targets as $target)
                try {
                    document.getElementById(`user-{{ $target }}`).checked = true;
                } catch (error) {
        ...
        function setSelectedUsers(selectedUserIds) {
