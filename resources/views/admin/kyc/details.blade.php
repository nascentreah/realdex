...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            ...
            @if ($extra_fields != null && count($extra_fields) > 0)
                <h6 class="card-sub-title my-5">{{ __('Custom Fields') }}</h6>
                <ul class="data-details-list">
                    @foreach ($extra_fields as $key => $value)
                        <li>
                            <div class="data-details-head">{{ ucfirst($key) }}</div>
                            <div class="data-details-des text-dark">{!! $value['field_name'] ? _x(strip_tags($value['field_name'])) : $space !!}
                            </div>
                        </li>{{-- li --}}
                    @endforeach
                </ul>
...
@push('breadcrumb-plugins')
    <a href="{{ route('admin.kyc.list') }}" class="btn btn-outline-secondary"><i class="bi bi-chevron-left mr-1"></i>
        {{ __('Back') }}</a>
@endpush
