        ...
        return [
            ...
            Column::make("Investment Plan", "investment_id")
                ...
                ->format(function ($value, $row, Column $column) {
                    if ($row->investment) {
                        return '<a href="' . route('admin.forex.investment.edit', $row->investment_id) . '" class="badge bg-primary">' . ucfirst($row->investment->title) . '</a>';
                    } else {
                        return '<span class="badge bg-danger">' . __('Investment Not Found') . '</span>';
                    }
                })
                ->html(),
        ...
        return [
            SelectFilter::make('Status')
                ...
                ->filter(function (Builder $builder, string $value) {
                    $builder->where('forex_logs.status', $value);
                }),
            DateFilter::make('From')
                ...
                ->filter(function (Builder $builder, string $value) {
                    $builder->where('forex_logs.created_at', '>=', $value);
                }),
            DateFilter::make('To')
                ->filter(function (Builder $builder, string $value) {
                    $builder->where('forex_logs.created_at', '<=', $value);
