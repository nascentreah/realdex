    ...
    public function show($id = '', $type = '')
    {
        ...
        if ($type == 'kyc_details') {
            if ($id == '') {
                ...
                if ($kyc) {
                    $extra_fields = json_decode($kyc->extra_field, true);
                    return view('admin.kyc.details', compact('kyc', 'extra_fields'))->render();
                } else {
                    return redirect()->route('admin.kyc.list')->withNotify(['error', 'KYC not found']);
                }
            }
        }
    }
